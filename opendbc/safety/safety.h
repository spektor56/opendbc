...
bool gear_park = false;  // true if the gear is in park, false otherwise
bool lkas_enabled_synced = false;  // true if lkas_enabled is synced with the car state
...
static void generic_rx_checks(void) {
  ...
  if (brake_pressed) {
    ...
    if (brake_pressed_prev && lkas_enabled && !gear_park) {
